...
func TestUpsertHomeBase_DailyLimit(t *testing.T) {
	...
	// Same day, different coordinates -> blocked.
	if _, err := store.UpsertHomeBase(ctx, user.ID, 320000000, 1220000000, now+1000); err != ErrHomeBaseLimited {
		t.Fatalf("UpsertHomeBase(second) error = %v, want ErrHomeBaseLimited", err)
	}

	// Same day, same coordinates -> idempotent OK.
	if _, err := store.UpsertHomeBase(ctx, user.ID, 310000000, 1210000000, now+2000); err != nil {
